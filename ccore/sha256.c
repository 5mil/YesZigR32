 ...
 * All rights reserved.
 *
 * Vendored from LuckyPepeChain/luckypepe-chain src/crypto/yescrypt/sha256.c
 * BSD 2-Clause License — see file header.
 */
...
#define RND(a,b,c,d,e,f,g,h,k) \
	h+=S1(e)+Ch(e,f,g)+k; d+=h; h+=S0(a)+Maj(a,b,c); y_xor_z=x_xor_y;
    ...
    uint32_t W[static restrict 64], uint32_t S[static restrict 8])
{
	...
	for (i = 0; i < 64; i += 16) {
		...
		RNDr(S,W,0,i); RNDr(S,W,1,i); RNDr(S,W,2,i); RNDr(S,W,3,i);
		RNDr(S,W,4,i); RNDr(S,W,5,i); RNDr(S,W,6,i); RNDr(S,W,7,i);
		RNDr(S,W,8,i); RNDr(S,W,9,i); RNDr(S,W,10,i); RNDr(S,W,11,i);
		RNDr(S,W,12,i); RNDr(S,W,13,i); RNDr(S,W,14,i); RNDr(S,W,15,i);
		if (i==48) break;
    ...
    uint32_t tmp32[static restrict 72])
{
	...
	if (len < 64-r) { memcpy(&ctx->buf[r], src, len); return; }
...
{
	uint32_t tmp32[72]; uint8_t pad[64]; uint8_t khash[32];
	_HMAC_SHA256_Init(ctx, _K, Klen, tmp32, pad, khash);
	insecure_memzero(tmp32,288); insecure_memzero(khash,32); insecure_memzero(pad,64);
}

static void _HMAC_SHA256_Update(HMAC_SHA256_CTX *ctx, const void *in, size_t len,
    uint32_t tmp32[static restrict 72])
{
...
void HMAC_SHA256_Final(uint8_t digest[32], HMAC_SHA256_CTX *ctx)
{
	...
	insecure_memzero(tmp32,288); insecure_memzero(ihash,32);
}
    ...
    size_t saltlen, uint64_t c, uint8_t *buf, size_t dkLen)
{
	...
	uint32_t tmp32[72];
	union { uint8_t tmp8[96]; uint32_t state[8]; } u;
	size_t i;
	uint8_t ivec[4];
	uint8_t U[32];
	uint8_t T[32];
